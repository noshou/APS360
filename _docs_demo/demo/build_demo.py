...
def sh_proj(
    ...
    """Reconstruct the angular scattering surface in Cartesian coords.

    r = 1 + Re(Sum B_lm Y_lm)
    ...
    Y_grid : ndarray of complex128, shape (K, H, W)
        Precomputed spherical harmonics evaluated on the (THETA, PHI)
        meshgrid.
...
# Precompute Y_lm on the fixed meshgrid once for all structures and all
# q-values.
K = (lMax + 1) ** 2
Y_grid = np.zeros((K, *THETA.shape), dtype=np.complex128)
for deg in range(lMax + 1):
    for m in range(-deg, deg + 1):
        Y_grid[deg * deg + deg + m] = sph_harm_y(deg, m, THETA, PHI)

# Build a (K,) index array mapping full k -> reduced k and sign for
# negative m.
_full_to_reduced = np.zeros(K, dtype=int)
_full_sign = np.ones(K, dtype=int)
for deg in range(lMax + 1):
    for m in range(-deg, deg + 1):
        k_full = deg * deg + deg + m
        abs_m = abs(m)
        _full_to_reduced[k_full] = deg * (deg + 1) // 2 + abs_m
        if m < 0:
            _full_sign[k_full] = (-1) ** abs_m


def expand_coeffs(B_lm_q: np.ndarray) -> np.ndarray:
...
for npz_file in sorted(PREC_DIR.glob("*.npz")):
    ...
    fig_iq.update_layout(
        title=dict(
            text="𝐼(𝑞)",
            font=dict(family="STIX Two Math, Georgia, serif", size=15),
        ),
    ...
    # B_lm frames - normalize to unit amplitude; track true max range
    # across all frames
    colorbar_cfg = dict(
        thickness=14, len=0.6, title=dict(text="z", font=dict(size=11))
    )
    frames = []
    max_r = 0.0
    all_xyz = []
...
.tabs {{
    display: flex; flex-wrap: wrap; gap: 4px; padding: 0 16px;
...
.panel-layout {{
    ...
    grid-template-rows:
        auto clamp(380px, 54vh, 600px) clamp(180px, 24vh, 280px) auto;
    column-gap: 12px;
    row-gap: 8px;
}}
.blm-caption {{ grid-column: 1; grid-row: 1; }}
.blm-plot    {{
    grid-column: 1; grid-row: 2; border: 1px solid var(--border);
...
.mol-viewer  {{
    grid-column: 2; grid-row: 2 / 4; width: 100%;
    border: 1px solid var(--border); border-radius: 6px;
    position: relative; background: var(--mol-bg);
}}
.blm-caption {{
...
.math-desc {{
    color: var(--text-dim); margin: 2px 0 2px 18px; font-size: 0.91em;
}}
.citation {{
    margin-top: 6px;
    font-size: 0.85em;
...
<p>The scattering intensity is expressed as a sum over spherical harmonic
channels (truncated at l = 50, giving 2601 modes):</p>
<div class="math-block">I(q) = 4π · Σ_{{l=0}}^{{50}} Σ_{{m=-l}}^{{l}}
|B_lm(q)|²</div>
<p>where the coefficients B_lm(q) are computed by projecting atomic
scattering onto the spherical harmonic basis:</p>
<div class="math-block">B_lm(q) = Σ_i  f_i(q) · j_l(q·rᵢ) ·
Y*_lm(θᵢ, φᵢ)</div>
<div class="math-desc">f_i(q): complex atomic form factor (f₀ + f₁ + i·f₂)
at momentum transfer q</div>
<div class="math-desc">j_l: spherical Bessel function of order l; encodes
radial shell information</div>
<div class="math-desc">Y_lm: complex spherical harmonic; * denotes
conjugate (analysis projection)</div>
<div class="math-desc">rᵢ, θᵢ, φᵢ: spherical coordinates of atom i
relative to the molecular centroid</div>
<p style="margin-top:8px">The 3D surface visualises the angular
scattering envelope:</p>
<div class="math-block">r(θ,φ) = 1 + Re( Σ_{{l,m}} B_lm(q) · Y_lm(θ,φ) )
/ max|B_lm(q)|</div>
<div class="math-desc">Deformations from a unit sphere indicate
anisotropic scattering at that q. Coefficients are normalised to unit
amplitude so the shape reflects anisotropy, not absolute intensity.</div>
<p style="margin-top:8px">Complexity: O(N · L²) vs O(N²) for the Debye
sum, orders of magnitude faster for large molecules.</p>
</details>
...
structures.forEach((s, i) => {{
    ...
    panel.innerHTML = `
        <div class="panel-layout">
            <div class="blm-caption">𝑟(θ,φ) = 1 + Re(Σ<sub>lm</sub>
            𝐵<sub>lm</sub>(𝑞) · 𝑌<sub>lm</sub>(θ,φ)) /
            max|𝐵<sub>lm</sub>|</div>
            <div id="blm-${{i}}" class="blm-plot"></div>
            <div id="iq-${{i}}" class="iq-plot"></div>
            <div class="citation">Source: <a href="${{s.citation_url}}"
            target="_blank" rel="noopener">${{s.citation_label}}</a></div>
...
function showTab(i) {{
    document.querySelectorAll('.tab-btn').forEach(
        (b, j) => b.classList.toggle('active', i === j)
    );
    document.querySelectorAll('.tab-panel').forEach(
        (p, j) => p.classList.toggle('active', i === j)
    );
    initTab(i);
}}
...
function initTab(i) {{
    ...
    fetch(s.data_url)
        ...
        .then(d => {{
            ...
            const iqPromise = Plotly.newPlot(
                'iq-' + i, d.iq.data, iqLayout,
            ...
            const blmPromise = Plotly.newPlot(
                blmDiv, d.blm.data, blmLayout,
                {{responsive: true, displayModeBar: false}}
