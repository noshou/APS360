"""CPU-only smoke test for the baseline evaluation pipeline, using
synthetic data.
...
_BASELINE_FACTORIES = {
    ...
    "mlp": lambda cfg, q, e, train: Mlp2Baseline(
        hidden=tuple(cfg["mlp_hidden"]), epochs=cfg["mlp_epochs"]
    ).fit(train),
...
def _make_synthetic_batch(
    ...
    """Build n_mols synthetic molecules; derive noisy I(q) via
    BinnedDebyeBaseline.

    Coordinates are drawn from a Gaussian point cloud (not a real force
    field -- only meant to give BinnedDebyeBaseline a plausible
    pairwise-distance distribution to work with).
    """
    vocabs, coords = [], []
    for _ in range(n_mols):
        ...
        vocabs.append(
            torch.tensor([VOCAB[e] for e in elements], dtype=torch.long)
        )
...
def _chunk(batch: Batch, size: int) -> list:
    """Split one big Batch into smaller Batches (mimics the real Batcher)."""

    n = batch.vocab.shape[0]
    return [
        Batch(
            vocab=batch.vocab[i : i + size],
            iqval=batch.iqval[i : i + size],
            coord=batch.coord[i : i + size],
        )
...
def main(config_path: str) -> None:
    ...
    print(
        f"Generating {cfg['n_train']} train + {cfg['n_test']} test "
        f"synthetic molecules ({cfg['atom_count_min']}-"
        f"{cfg['atom_count_max']} atoms, ions={cfg['ions']})..."
    )
    train_full = _make_synthetic_batch(
        cfg, cfg["n_train"], q_grid, energy, gen, rng
    )
    test_full = _make_synthetic_batch(
        cfg, cfg["n_test"], q_grid, energy, gen, rng
    )
    ...
    for key in cfg["baselines"]:
        if key not in _BASELINE_FACTORIES:
            raise ValueError(
                f"Unknown baseline {key!r} in config; "
                f"known: {list(_BASELINE_FACTORIES)}"
            )
        ...
        print(
            f"  {name:<26s}  MSLE={result.msle:.4f}  "
            f"R²(raw)={result.r2_raw:.4f}  "
            f"R²(log1p)={result.r2_log1p:.4f}  "
            f"{result.us_per_atom:.2f} μs/atom"
        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(__file__), "baselines_smoke_test.yaml"
        ),
    )
    args = p.parse_args()
    main(args.config)
