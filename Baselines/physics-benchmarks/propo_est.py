...
class PropoEstBaseline(Baseline):
    ...
    """
    Deterministic propoEst estimator of the Debye sum (port of SaxsEst propoEst).

    Replaces the per-pair form-factor product in the exact Debye equation with a
    single q-dependent scalar weight ``wEst(q)`` from the atom-type frequency
    table, then sums ``sinc(q r_ij)`` over all pairs. Form factors are treated as
    real magnitudes ``|f(q)|`` (imaginary part cancels by pair symmetry).
    ...
    def __init__(
        ...
        """qgrid: (Q,) q-points [1/A]. energy: X-ray energy [eV]. e: propoEst epsilon."""
        self._qgrid      = qgrid
