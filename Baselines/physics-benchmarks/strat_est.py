...
class StratEstBaseline(Baseline):
    ...
    Allocation": atoms are split into heavy/light strata by |f(q0)|, the sample
    budget ``ceil(a*m)`` is split between the strata in proportion to their mean
    |f(q0)|, and each stratum is then sampled uniformly with replacement.

    Only the off-diagonal (i != j) part of the Debye sum is estimated from the
    sample; the diagonal ``sum_i |f_i(q)|^2`` is computed exactly over all m
    atoms, since it is O(m) and dominates I(q) at high q where the off-diagonal
    terms cancel. This mirrors ``PropoEstBaseline``.

    Hansen-Hurwitz weights
    ----------------------
    Each draw in stratum h is uniform over that stratum's ``pop_h`` atoms, so the
    per-draw selection probability is ``p = 1/pop_h`` and the HH weight of a
    sampled atom is ``w_h = pop_h / n_h``. Sampling is WITH replacement, so a
    sample pair ``(k, l)`` that happens to land on the same atom is a diagonal
    term, not an off-diagonal one: those coincident pairs are masked out and the
    surviving within-stratum pairs rescaled by ``n_h / (n_h - 1)``, which makes
    the off-diagonal estimate unbiased. Cross-stratum pairs need no rescaling.
    ...
    def __call__(self, batch: Batch) -> Float[torch.Tensor, "N Q"]:
        ...
        for n in range(batch.coord.shape[0]):
            ...
            # cap the budget (not the drawn sample) so the HH weights below stay
            # consistent with the number of draws actually made
            total_samples = min(int(torch.ceil(torch.tensor(self._A * m)).item()), self._MAX_M)
            ...
            # exact diagonal over ALL m atoms: sum_i |f_i(q)|^2 (O(m), no sampling)
            diag = (ftable[vocab_n] ** 2).sum(dim=0)      # (Q,)

            # A stratum drawn fewer than twice forms no within-stratum pair, so its
            # entire off-diagonal would silently drop out of the estimate. On small
            # molecules ceil(a*m) is too small a budget to keep both strata above
            # that floor, so degrade to a single unstratified stratum (same weights,
            # same estimator) rather than lose one stratum's pairs altogether.
            strata = ((heavy_atom, heavy_samples), (light_atom, light_samples))
            if (bool(heavy_atom.any()) and bool(light_atom.any())
                    and (heavy_samples < 2 or light_samples < 2)):
                strata = ((torch.ones_like(heavy_atom), total_samples),
            ...
            for sid, (atom_mask, n_draw) in enumerate(strata):
                ...
                # Uniform draw with replacement over the stratum pool. The old
                # type-frequency inverse-CDF draw was a no-op: picking a type with
                # P = cnts[t]/pop and then a uniform atom among that type's cnts[t]
                # members gives P(atom) = (cnts[t]/pop) * (1/cnts[t]) = 1/pop, so
                # cnts[t] cancels and every atom is equally likely regardless of
                # type. gen is a CPU generator (for reproducibility), so the draw
                # is made on CPU and moved onto the batch's device to index there.
                pick = torch.randint(pop, (n_draw,), generator=gen)              # (n_draw,)
                samp_idx.append(idx_pool[pick.to(device)])

                w_h[sid] = pop / n_draw                                          # p = 1/pop => w = pop/n
                # n_draw == 1 forms no distinct sample pair, so that stratum's own
                # off-diagonal is not estimable (only reachable for tiny m); the
                # (hh - self) term it scales is identically 0 there anyway.
            ...
            # Collapse repeat draws. Sampling is with replacement, so the n_h draws
            # land on only ~(1 - e^-a) * pop_h distinct atoms; folding each atom's
            # multiplicity r_i into its amplitude (G_i = r_i * g_i) is algebraically
            # exact and shrinks the dominant (Qc, U, U) kernel to ~0.56x its (S, S)
            # size at a = 0.6. Strata partition the atoms, so no unique() bucket ever
            # mixes draws from different strata.
            sel, rep = torch.unique(torch.cat(samp_idx), return_counts=True)      # (U,), (U,)
            ...
            # G_i(q) = r_i * w_i * |f_i(q)|: each q's off-diagonal estimate is a
            # quadratic form in G with the sinc kernel, split into heavy/light blocks.
            G   = ftable[vocab_n[sel]] * (rep.to(qgrid.dtype) * w_u).unsqueeze(1)  # (U, Q)
            U   = dist.shape[0]
            ...
            for a, b in q_chunks(qgrid.shape[0], U):
                ...
                qd   = qgrid[a:b].view(-1, 1, 1) * dist.unsqueeze(0)    # (Qc, U, U)
                ...
                gh   = gc * m_h                                         # (Qc, U)
                gl   = gc * m_l                                         # (Qc, U)
                out  = torch.bmm(sinc, torch.stack((gh, gl), dim=2))    # (Qc, U, 2)

                hh   = (gh * out[..., 0]).sum(dim=1)                    # (Qc,) heavy-heavy, incl. i==j
                ll   = (gl * out[..., 1]).sum(dim=1)                    # (Qc,) light-light, incl. i==j
                hl   = (gh * out[..., 1]).sum(dim=1)                    # (Qc,) heavy-light (i!=j always)

                # drop each block's own i==j term (the diagonal is exact, above), then
                # rescale the within-stratum pairs; cross-stratum pairs are unbiased
                off  = (cvec[0] * (hh - (gh ** 2).sum(dim=1))
