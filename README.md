...
**Step 1 (`_query_buckets`):** for each `(min_atoms, max_atoms)` range in `DEFAULT_BUCKETS` (60 entries), molecules are queried sorted by atom count, unsplit by `atom_size_ceil` yet.

**Step 2 (`_family_split_assignment`):** every molecule is assigned to exactly one of train/val/test at the FAMILY level, not individually - fixing a leak where near-duplicate molecules landed on both sides of the split, letting the model "generalize" to val/test points it had effectively already memorized in train. A family is a connected component under two union signals: exact-composition duplicates (same element sequence, same source group - e.g. a structure re-deposited/redetermined under a different id: `mofs`'s `core_ADEGIA`/`core_ADEGIA01`, CSD refcodes for the same compound) detected via a hashed composition signature (blake2b digest of the sorted VOCAB_idx sequence - not the raw sequence itself, since on this dataset atom-count coincidentally collides within a group for almost the entire ~1M-row dataset, so holding every candidate's raw element sequence at once is several GB; a 16-byte digest is what stays resident, ~30s/~1.5GB peak for the full dataset), and, for a fixed allowlist of parametric-family datasets only (`_FAMILY_PREFIX_GROUPS`: `hydration_shells`, `ar_ne_clusters`, `(NaCl)_nCl-`, `si_ge_clusters`, `binary_clusters`, `(Na,Co,Ag,Pb,Mo,Fe)_monoatomic_clusters`), same stem-prefix-before-first-digit (a parametric series varying shell size, cluster size, isomer, or stoichiometry, e.g. `Ar_10`/`Ar_11`/`Ar_10Cl2` -> family `Ar_`). The prefix rule is scoped to that allowlist deliberately: on an id/catalog group like `COD` or `rcsb_sml` (e.g. `COD_0`, `COD_1`, ...) the same rule would collapse the entire group into one family, since the catalog number sits right after a fixed prefix - there the composition-duplicate signal alone is what's meant to catch leakage. Families are shuffled with `batcher_seed` and greedily filled toward 70/15/15 by molecule count (largest-remaining-deficit bin packing), so proportions still land close to target even though family sizes vary.

**Step 3 (`_split_batch` + `_collapse_tiny_batches`, per bucket per split):** each bucket's rows are filtered to their assigned split, then chunked to <= `atom_size_ceil` (default: 3x largest molecule) via binary search on prefix sums (same algorithm as before, just applied per split now). Batches with < 3 molecules are merged into their nearest same-split neighbour by median atom count. Train/val/test batch counts are no longer forced equal (previously every bucket contributed one sub-batch to every split; now a split's batches come only from buckets/rows actually assigned to it), so `len(train_loader) != len(val_loader) != len(test_loader)` in general.
...
| `loss_per_epoch.{png,json}`          | run-level | free — train/val/test loss vs epoch, through this epoch; rebuilt by reading every earlier epoch's`metrics.json` off disk. Written at the root of`data_dir`, not nested per-epoch, since it's one running trend not a per-epoch snapshot |
| `pct_err_per_epoch.png`              | run-level | free — same as above but train/val/test mean absolute percent error (raw I(q) space); no separate JSON, the numbers already live in`loss_per_epoch.json`'s records                                                                    |
...
| `lr_patience`       | 4       | Consecutive non-improving epochs tolerated before the LR is cut.                                                                                                                                                                         |
...
| `smoothing_lr_cut_trigger` | 2 | Consecutive LR cuts (no genuine improvement between them) before each `lambda_7` ramp step fires - same threshold the old on/off smoothing switch used. See "Staged smoothing + auto-kill" below.                                                                                                                                                                                                                    |
| `smoothing_n_stages` | 4      | Number of equal steps from `lambda_7=0` up to its target value, one per `smoothing_lr_cut_trigger`-plateau. Fully ramped + one more plateau = fully converged.                                                                                                                                                                          |
...
### Staged smoothing + auto-kill

`SplineSmooth` always runs (`forward()` calls it unconditionally). `LambdaHead` is bias-initialized to Λ≈0 (near-identity, `softplus(-10) ~= 4.5e-5`) and learns from the reconstruction loss the whole run - it is never a cold-started subnetwork. What's staged instead is `lambda_7`'s roughness penalty: it starts at 0 and steps up toward its configured target in `smoothing_n_stages` equal increments, so `OutputHead` is nudged toward a smoother raw curve gradually rather than via one instant full-strength penalty (which, combined with an untrained `LambdaHead`, used to blow up training on trigger).
...
`Train/train.py` tracks `ReduceLROnPlateau`'s LR cuts, CONSECUTIVELY: a cut followed by a genuine new `best_val_ema` means it worked, so the streak resets to 0 - not a cumulative "N cuts ever" count. Each time `smoothing_lr_cut_trigger` cuts (default 2) happen back-to-back with no escape, that's the "genuinely stuck at this lambda_7 level" signal: weights + optimizer roll back to `ckpt_best_ema`, `lambda_7` steps up by `1/smoothing_n_stages` of its target, LR resets to `lr` (scheduler still free to cut again from there), and training continues. Once `lambda_7` is fully ramped and the same plateau condition fires again, that means it's fully converged and the run stops.
...
On full convergence, `Train/train.py` destroys the vast.ai instance itself (`_destroy_vast_instance`, via the `vastai` CLI + `CONTAINER_ID`/`VAST_API_KEY` from the environment) - safe because every checkpoint/log/plot has already been pushed to Drive per-epoch by that point. See `_shell_scripts/README.md`'s "Auto-kill on convergence" section for setup.
