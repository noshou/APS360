...
Two decay mechanisms run together, one unconditional and one reactive:

**Per-epoch decay** - `torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=lr_gamma)`, stepped once per epoch (not per batch): fires unconditionally regardless of how training is going. `lr_gamma=1.0` disables it.

**Reactive mid-epoch cut** - unlike the per-epoch decay, this only fires on an actual stall: every `plateau_window` batches, rank 0's mean training loss over that window is compared to the best window seen since the last cut. `plateau_patience` consecutive non-improving windows triggers an in-place `lr *= plateau_factor` on every rank's optimizer (the bad-streak counter resets to 0 after a cut, doubling as the cooldown - no separate knob needed). This exists because a single epoch here takes hours, so waiting for an epoch boundary to react to a stall that develops mid-epoch wastes a lot of compute. `plateau_factor=1.0` disables it. Only rank 0's loss stream feeds the window: under DP routing, each rank sees a different molecule shard, so per-rank means would disagree on when to cut, and "which rank is right" isn't a meaningful question - it would just desync the ranks' LRs. The window-closure trigger (`_bi % plateau_window`) is deterministic and identical across ranks (they walk the same shuffled `train_loader` in lockstep), so every rank enters the check together even though only rank 0 decides; the decision is broadcast (`dist.broadcast`) so every rank applies the identical cut.

Because `ExponentialLR.step()` in this torch version multiplies the optimizer's *current* lr by gamma (not a fixed base lr), rather than recomputing from scratch, it composes correctly with the plateau cut automatically: whichever mechanism last touched `param_groups` wins, and both end up captured together in `optimizer.state_dict()`.

**Resume**: `lr_gamma`'s schedule is keyed off the absolute epoch number rather than a saved scheduler counter - `start_epoch` already correctly encodes epochs-completed-so-far across both epoch-boundary and mid-epoch resumes (see `Train/train.py`'s resume block), so a horizon-based schedule (cosine annealing, etc.) can't be used safely here anyway, since `cfg.epochs` is only the current invocation's epoch count, not the run's total horizon (a resume runs `cfg.epochs` MORE epochs from wherever it left off). Because both mechanisms mutate `optimizer.param_groups` in place, a checkpoint saved by this code already has the *true* current lr baked into `optimizer.state_dict()` - no scheduler state needs to live in the checkpoint for the per-epoch decay. The one exception: a checkpoint saved *before* this scheduler existed has an lr that was never touched in place (still exactly `cfg.lr`), so on that specific resume the scheduler is fast-forwarded by `start_epoch - 1` steps to reach the value it should be at; this fast-forward is skipped for any checkpoint already written by this code; doing it unconditionally would double-apply decay/cuts that already happened before the crash. The reactive cut's own state (`plateau_best`, `plateau_bad_streak` - how close a run was to its next cut) genuinely can't be reconstructed after the fact, so those two values ARE saved in the resume checkpoint and restored on load.
...
| `plateau_factor`    | 0.5     | Multiplicative lr cut on a detected plateau. 1.0 = disabled.                                                                                    |
