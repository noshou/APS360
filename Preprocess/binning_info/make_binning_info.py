...
# Fixed bins, same boundaries as before, recompute stats against new
# distribution
BINS = [
    (1, 3),
    (4, 6),
...
hdr = (
    f"  {'Group':<44} {'Molecules':>9}  {'Atoms total':>12}  "
    f"{'Min':>6}  {'Max':>6}  {'Mean':>6}"
)
lines.append(hdr)
lines.append("  " + "─" * 86)
...
lines.append(
    "Rationale: loss is per I(q) curve, so molecule count determines "
    "training signal."
)
...
lines.append("")
lines.append(
    f" {'Bin':>3}  {'Range':<16}  {'Molecules':>9}  {'Total atoms':>12}  "
    f"{'Mean':>8}  {'p95':>8}  {'Max':>8}  {'Pad':>5}"
)
lines.append(
    f" {'─' * 3}  {'─' * 16}  {'─' * 9}  {'─' * 12}  "
    f"{'─' * 8}  {'─' * 8}  {'─' * 8}  {'─' * 5}"
...
for i, (lo, hi) in enumerate(BINS):
    if mol_counts[i] == 0:
        continue
    lines.append(
        f" {i + 1:>3}  ({lo:>7,},{hi:>7,})  {mol_counts[i]:>9,}  "
        f"{atom_totals[i]:>12,}  {bin_means[i]:>8.1f}  "
        f"{bin_p95[i]:>8.1f}  {bin_max[i]:>8,}  {pad_ratios[i]:>5.2f}"
    )

lines.append("")
lines.append(
    f"Molecule count range: {min_mol:,} – {max_mol:,}  (mean {mean_mol:,})"
)
lines.append(
    f"Pad ratio range:      {min_pad:.2f} – {max_pad:.2f}  "
    f"(mean {mean_pad:.2f})"
)
lines.append(f"All bins >= 0.60 pad: {all_pass}")
lines.append("")
lines.append("NOTE ON atom_size_ceil:")
lines.append(
    f"  atom_size_ceil is set to {atoms_all.max():,}, "
    "all bins are within this ceiling."
)
lines.append("")
lines.append("=" * 80)
lines.append("FILES")
lines.append("=" * 80)
lines.append(
    "  atom_count_histogram.png - full distribution, all groups, bin lines"
)
